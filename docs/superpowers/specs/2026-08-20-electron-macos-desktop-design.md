...
- Store Livecraft desktop settings only in browser `localStorage` or `sessionStorage`.
...
The packaged app persists Livecraft-owned desktop settings to a well-known, user-owned configuration directory rather than relying only on browser `localStorage` or `sessionStorage`.

- On macOS, use Electron's `app.getPath('userData')`, which resolves to `~/Library/Application Support/Pi Livecraft` (subject to the configured application name).
- On future Linux and Windows builds, use the platform-specific directory supplied by Electron's `userData` path rather than hard-coding a macOS path or a home-directory dotfile.
- Store settings in a versioned, human-inspectable JSON file with restrictive user-only permissions where the platform supports them.
- Persist at least the configured Pi executable path, desktop tab/window restoration state, and any desktop-only connection or launch preferences. Do not persist per-launch backend secrets, session credentials, or Pi-owned configuration.
- Keep browser preferences compatible during migration: read the current browser storage as a one-time fallback, write the canonical desktop value to disk, then prefer disk-backed values thereafter. No browser data is silently deleted.
- Renderer code accesses disk-backed preferences only through a narrow, typed Electron bridge/API. The renderer never receives arbitrary filesystem access.

The existing browser build can retain its browser-local preference storage. A shared preference contract should distinguish portable UI preferences from desktop-owned settings and preserve current behavior when Electron is unavailable.
...
- Desktop settings resolve to Electron's platform user-data location, serialize safely, migrate browser fallback values once, and never persist ephemeral secrets.
...
6. A configured Pi path and desktop tab/window state survive an app restart in the documented Application Support location; per-launch secrets do not.
7. Quit does not leave Livecraft backend/supervisor processes behind and follows manager shutdown behavior.
